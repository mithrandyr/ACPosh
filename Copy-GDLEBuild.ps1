...
foreach ($f in $filesToCopy) {
    Join-Path -Path $BuildFolder -ChildPath $f | Copy-Item -Destination $ServerFolder
}
...
Get-ChildItem -Path $BuildDataFolder -Recurse -File -Name |
    ForEach-Object {
        Copy-Item -Path (Join-Path $BuildDataFolder $_) -Destination (Join-Path $ServerDataFolder $_) -Force
    }