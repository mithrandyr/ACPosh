Param([string]$FilePath = "E:\games\ACServer\Data\json\spells.json", [string]$OutputPath = "$PSScriptRoot\spellData.json")
...
foreach($spell in $spells[0..500]) {
    ...
    $spellInfo = [PSCustomObject]@{
        ...
        SpellLevel = $spellAdditional.SpellLevel
    ...
    $spellList.Add($spellInfo) | Out-Null
}
